...
public class SignUpImpl implements SignUp {
    private AccountDAOFactory dao;

    public SignUpImpl(AccountDAOFactory accountDaoFactory) {
        this.dao = accountDaoFactory;
    }
    ...
    public byte checkEmail(String email) {
        ...
        if (dao.getAccountDAO().getAccountByEmail(email) != null) {
            if (email.equals(dao.getAccountDAO().getAccounts().get(email).getEmail())) {
    ...
    public byte SignUp(String username, String email, String password) {
        ...
        dao.getAccountDAO().addAccount(new Account(username, email, password));
